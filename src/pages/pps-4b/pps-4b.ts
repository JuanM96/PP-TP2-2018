...
/**
 * Generated class for the Pps_4bPage page.
 *
...
@Component({
  selector: 'page-pps-4b',
  templateUrl: 'pps-4b.html',
})
export class Pps_4bPage {
  usuario:string
  mensaje:string
  ...
  ionViewDidEnter(){
    this.coleccionTipadaFirebase= this.objFirebase.collection<chat>('PPS-4B', ref=> ref.orderBy('tiempo')); 
  ...
  agregarMensaje(mensaje:string)
  {
   ...
   this.objFirebase.collection<chat>('PPS-4B').add(objetoJsonGenerico).then(
    Retorno=>
    {
